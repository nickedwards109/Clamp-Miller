class ClampMaker
	...
	def create_toolpaths
		...
		def create_hole_toolpath
			...
			remaining_material = @material_thickness
				...
				while (remaining_material + @tool_radius) > 0
					...
					remaining_material = remaining_material - @axial_depth_of_cut
				...
				def create_XY_hole_profile_toolpath
					...
					remaining_hole_stock = @hole_diameter - 2*@tool_radius

					#set a counting variable for use within the circular toolpath generation loop
					i = 1

					#rough out the hole diameter until there is less than one tool radius of stock remaining on each opposing side
					while (remaining_hole_stock/2) > @tool_radius
						...
						remaining_hole_stock = remaining_hole_stock - 2*@tool_radius
		...
		def create_slot_toolpath
			...
			remaining_material = @material_thickness
				...
				def create_XY_slot_profile_toolpath
					...
					#feed to the Y position at the opposite end of the slot
					...
					while (remaining_slot_stock/2) > @tool_radius
						...
						remaining_slot_stock = remaining_slot_stock - 2*@tool_radius
		...
		def create_outer_profile_toolpath
			...
			puts "G0X#{(@width + @tool_radius).round(3)}Y#{(@small_profile_radius + @tool_radius).round(3)}"
			...
			remaining_material = @material_thickness
				...
				def create_XY_outer_profile_toolpath
					...
					puts "G2X#{(@width - @small_profile_radius).round(3)}Y-#{@tool_radius.round(3)}I-#{@small_profile_radius + @tool_radius}J0.0F#{@xy_feedrate}"
					...
					puts "G1X#{(@width + @tool_radius).round(3)}Y#{(@small_profile_radius + @tool_radius).round(3)}F#{@xy_feedrate}"
